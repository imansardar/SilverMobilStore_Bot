...
def start(message):
    ...
    try:
        if is_member(user_id):
            # منو رو نشون بده
            bot.send_message(user_id, "خوش اومدی!", reply_markup=main_menu(user_id))
        else:
            bot.send_message(user_id, "لطفاً عضو کانال بشید.", reply_markup=join_channel_button())
    except Exception as e:
        # اگه خطایی توی بررسی عضویت پیش اومد، به جای ارور، کاربر رو به کانال بفرست
        bot.send_message(user_id, "برای استفاده از ربات، لطفاً عضو کانال بشید.", reply_markup=join_channel_button())
